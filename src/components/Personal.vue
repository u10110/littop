...
async function removeProfileImage(kind) {
  ...
  const label = isAvatar ? 'аватар' : 'обложку';
  if (!window.confirm(`Удалить ${label} из профиля?`)) return;
  ...
  try {
    await saveProfile(buildProfilePayload(isAvatar ? { avatarUrl: null } : { coverImageUrl: null }));
    syncProfileForm();
    resetProfileImageSelection(kind);
    profileImageSuccess.value = isAvatar
      ? 'Аватар удалён из профиля.'
      : 'Обложка удалена из профиля.';
  } catch (error) {
    profileImageError.value = error instanceof Error ? error.message : 'Не удалось удалить фото.';
  } finally {
      ...
      <section id="profile-images" class="cabinet-info">
        <article class="dash-card image-settings-card">
          ...
          <div class="profile-image-grid">
            <div class="profile-image-card"><b>Аватар</b><input ref="avatarFileInput" type="file" accept="image/*" @change="handleProfileImageChange('avatar', $event)"><div class="inline-actions"><button class="btn btn-primary" type="button" :disabled="profileImageBusy" @click="submitProfileImage('avatar')">Загрузить</button><button v-if="profileForm.avatarUrl" class="btn btn-outline" type="button" :disabled="profileImageBusy" @click="removeProfileImage('avatar')">Удалить аватар</button></div></div>
            <div class="profile-image-card"><b>Обложка</b><input ref="coverFileInput" type="file" accept="image/*" @change="handleProfileImageChange('cover', $event)"><div class="inline-actions"><button class="btn btn-secondary" type="button" :disabled="profileImageBusy" @click="submitProfileImage('cover')">Загрузить</button><button v-if="profileForm.coverImageUrl" class="btn btn-outline" type="button" :disabled="profileImageBusy" @click="removeProfileImage('cover')">Удалить обложку</button></div></div>
          </div>
        </article>
